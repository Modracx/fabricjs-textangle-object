/*!
 * fabric.TextAngle — diagonal / angle text-on-bezier effect
...
;(function (fabric) {
    ...
    const _DEFAULT_CTRL_PTS = [{ x:0, y:1 }, { x:1, y:0.4 }];

    fabric.TextAngle = fabric.util.createClass(fabric.IText, {
        type: 'text-angle',
        ctrlPts: null,
        kerning: 0,
        flipped: false,
        ...
        _updateCurve: function() {
            ...
            for(let i=0;i<n;i++){
                ...
                const pt=path.at(charCx); const hw=cw[i]/2;
                const dy0=this.flipped?-H*0.08:-asc, dy1=this.flipped?H*0.92:desc;
                if(pt.x-hw<minX)minX=pt.x-hw; if(pt.x+hw>maxX)maxX=pt.x+hw;
                if(pt.y+dy0<minY)minY=pt.y+dy0; if(pt.y+dy1>maxY)maxY=pt.y+dy1;
            }
        ...
        _render: function(ctx) {
            ...
            const {_chars:chars,_cw:cw,_path:path,_H:H,_L:L,_leftOff:leftOff}=this;
            ctx.save();
            ctx.font=this._getFontDeclaration();
            ctx.textBaseline='alphabetic'; ctx.textAlign='center';
            ctx.fillStyle=this.fill;
            ctx.translate(-(this._bbX||0),-(this._bbY||0));
            const yOff=this.flipped?H*0.72:0;
            let xPos=0;
            for(let i=0;i<chars.length;i++){
                const charCx=xPos+cw[i]/2+leftOff; xPos+=cw[i]+this.kerning;
                if(charCx<0||charCx>L)continue;
                const pt=path.at(charCx);
                if(this.stroke&&this.strokeWidth>0){ctx.strokeStyle=this.stroke;ctx.lineWidth=this.strokeWidth;ctx.strokeText(chars[i],pt.x,pt.y+yOff);}
                ctx.fillText(chars[i],pt.x,pt.y+yOff);
            }
            ctx.restore();
        },
    ...
    fabric.TextAngle.fromObject = function(object,callback,forceAsync){
        return fabric.Object._fromObject('TextAngle',object,callback,forceAsync);
    };
